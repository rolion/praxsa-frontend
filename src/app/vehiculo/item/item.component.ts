...
export class ItemComponent {
  ...
  getDataTableData(page: number, text: string){
    this.apiService.get('/product/carpart/search', `/?page=${page}&text=${text}`).subscribe(data=>{
      console.log(data)
  ...
  onAddRowSave(newBrand: UntypedFormGroup){
    ...
    this.apiService.post('product/carpart/', newBrand.value)
  ...
  onEditRowSave(brand: any){
    ...
    this.apiService.put(`product/carpart/${brand.value.id}/`, brand.value).subscribe(res=>{
      console.log(res)
      let page=this.offset +1
      this.getDataTableData(page, this.textToSearch)
      this.item.reset();
      this.modalService.dismissAll();
      this.updateRecordSuccess();
      this.loadingIndicator = false;
    })
  }
